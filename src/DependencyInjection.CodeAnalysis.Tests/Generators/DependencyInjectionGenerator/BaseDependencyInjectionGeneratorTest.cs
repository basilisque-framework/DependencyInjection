    ...
    where TGenerator : IncrementalSourceGeneratorVerifier<CodeAnalysis.DependencyInjectionGenerator>, new()
{
    ...
    public virtual async Task Test()
    {
        ...
        {
            verifier.TestState.GeneratedSources.Add((typeof(CodeAnalysis.DependencyInjectionGenerator), expectedSource.Name, SourceText.From(expectedSource.SourceText, Encoding.UTF8)));
        }

        await verifier.RunAsync();
    }

    protected virtual IncrementalSourceGeneratorVerifier<CodeAnalysis.DependencyInjectionGenerator> GetVerifier()
    {
        ...
        return (IncrementalSourceGeneratorVerifier<CodeAnalysis.DependencyInjectionGenerator>)verifier;
    }
    ...
    {
        var assemblyNameInfo = System.Reflection.Assembly.GetAssembly(typeof(CodeAnalysis.DependencyInjectionGenerator))?.GetName();
        ...
        result.Add(getRootNamespaceImplSrc(rootNamespace));
...
{{
    ...
    private (string, string) getRootNamespaceImplSrc(string? rootNamespace)
    {
        string filename;
        if (rootNamespace == null)
        {
